...
import { addTodo, removeTodo, toggleTodo } from '../redux/slices/todo-slice.ts';
...
describe('TodoPage', () => {
  ...
  it('does not add an empty todo', () => {
    ...
    const actions = store.getActions();
    expect(actions).not.toContainEqual(addTodo(''));
  });

  it('does not remove a non-existent todo', () => {
    render(
      <Provider store={store}>
        <Router>
          <TodoPage />
    ...
    const actions = store.getActions();
    expect(actions).not.toContainEqual(removeTodo(3));
  });
});